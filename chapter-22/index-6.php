<?php
// Пример: кеш ресурсов
// Локальное кеширование ресурса по идентификатору
class FileLogger {
    ...
    public static function create($fname) {
        // Вначале проверяем: возможно, объект для указанного имени файла уже существует?
        // Тогда его и возвращаем.
        if (isset(self::$loggers[$fname])) return self::$loggers[$fname];
        // А иначе создаем полностью новый объект и сохраняем ссылку на него в статическом массиве.
...
echo "{$logger1->getTime()}, {$logger2->getTime()}"; // время создания объектов должно совпадать